...
[JsonConverter(typeof(IdpUpdateKnownResponse.JsonConverter))]
[Serializable]
public class IdpUpdateKnownResponse
{
    private IdpUpdateKnownResponse(string type, object? value)
    {
        Type = type;
        Value = value;
    }
    ...
    public override bool Equals(object? obj)
    {
        ...
        if (ReferenceEquals(this, obj))
            return true;
        if (obj is not IdpUpdateKnownResponse other)
            return false;
    ...
    public static implicit operator IdpUpdateKnownResponse(
        Auth0.MyOrganizationApi.IdpAdfsResponse value
    ) => new("idpAdfsResponse", value);
    ...
    [Serializable]
    internal sealed class JsonConverter : JsonConverter<IdpUpdateKnownResponse>
    {
        public override IdpUpdateKnownResponse? Read(
        ...
        )
        {
            ...
            if (reader.TokenType == JsonTokenType.StartObject)
            {
                ...
                foreach (var (key, type) in types)
                {
                    try
                    {
                        ...
                        if (value != null)
                        {
                            IdpUpdateKnownResponse result = new(key, value);
                            return result;
                        }
                    }
                    catch (JsonException)
            ...
            throw new JsonException(
                $"Cannot deserialize JSON token {reader.TokenType} into IdpUpdateKnownResponse"
            );
        }

        public override void Write(
            Utf8JsonWriter writer,
            IdpUpdateKnownResponse value,
            JsonSerializerOptions options
        )
        {
            if (value == null)
        ...
        public override IdpUpdateKnownResponse ReadAsPropertyName(
            ref Utf8JsonReader reader,
            System.Type typeToConvert,
            JsonSerializerOptions options
        )
        {
            ...
            IdpUpdateKnownResponse result = new("string", stringValue);
            return result;
        }
